...
def form_fem(fem, opt):
    ...
    V = functionspace(mesh, ("Lagrange", 1, (dim,)))   # was VectorFunctionSpace(mesh, ("CG", 1))
    S0 = functionspace(mesh, ("DG", 0))                 # was FunctionSpace(mesh, ("DG", 0))
    S = functionspace(mesh, ("Lagrange", 1))            # was FunctionSpace(mesh, ("CG", 1))
    ...
    linear_problem = LinearProblem(u_field, lambda_field, lhs, rhs, opt["l_vec"],
        spring_vec, [bc], fem.get("gpu_petsc_options", fem["petsc_options"]),
        reanalysis_options=opt.get("reanalysis_options"))
