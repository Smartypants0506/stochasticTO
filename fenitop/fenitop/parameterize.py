...
class DensityFilter():
    ...
    def forward(self):
        ...
        if reason < 0 and MPI.COMM_WORLD.rank == 0:
            print(f"WARNING: filter forward solve diverged, reason={reason}", flush=True)
    ...
    def backward(self, sf_vectors):
        ...
        for sf in sf_vectors:
            if sf is not None:
                ...
                reason = self.solver.getConvergedReason()
                if reason < 0 and MPI.COMM_WORLD.rank == 0:
                    print(f"WARNING: filter backward solve diverged, reason={reason}", flush=True)
