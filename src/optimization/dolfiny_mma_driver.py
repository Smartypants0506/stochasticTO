...
def _retrain_pce_pair(
    ...
    Args:
        ...
        rho_current: [n_elems] design density at which to train (the
...
def run_robust_topopt(
    ...
    rf_heaviside = build_random_heaviside_from_function_space(
        rho_phys_field, fem["mesh_simplices"], random_heaviside_config,
    )
    ...
    def jacobian_inequality_callback(
        ...
        J.setValues(range(1), range(n_elems), dh_drho.reshape(1, -1))
        J.assemble()
